...
for (letter of data) {
  if (letter === "-") {
    ...
    dataNoSymbols = dataNoSymbols + letter;
  }
}

const dataArr = dataNoSymbols.split(" ");
...
//     if (dataArr[i][j] === dataArr[i - 1]) {
//       count++;
//     }
//   }

//   if (count >= dataArr[i - 3] && count <= dataArr[i - 2]) {
//     validCount++;
//     count = 0;
//   }
// }
...
for (let i = 3; i < dataArr.length; i += 4) {
  let num1 = dataArr[i - 3] - 1;
  let num2 = dataArr[i - 2] - 1;
  let letter = dataArr[i - 1];

  if (dataArr[i][num1] === letter && dataArr[i][num2] !== letter) {
    validCount++;
  }
