...
let passportsArr = data.split("\n\n");

let validPass = 0;
let validPassArr = [];

for (el of passportsArr) {
  if (
    el.includes("hcl") &&
    el.includes("eyr") &&
    el.includes("ecl") &&
    ...
    validPass++;
    validPassArr.push(el);
  }
}

let validPassObjArr = [];

for (let el of validPassArr) {
  let newStr = "";
  ...
  validPassObjArr.push(obj1);
}

let verifiedPass = 0;
//console.log(validPassObjArr.length);
let verified = [];
let unverified = [];

for (let el of validPassObjArr) {
  let byr = Number(el.byr);
  let byrValid = false;
  let iyr = Number(el.iyr);
  let iyrValid = false;
  let eyr = Number(el.eyr);
  let eyrValid = false;
  let hgt = el.hgt;
  let hgtSlice = Number(el.hgt.slice(0, -2));
  let hgtValid = false;
  let hcl = el.hcl;
  let hclValid = false;
  ...
  if (hgt.endsWith("cm")) {
    if (hgtSlice >= 150 && hgtSlice <= 193) {
      hgtValid = true;
    }
  }

  if (hgt.endsWith("in")) {
    if (hgtSlice >= 59 && hgtSlice <= 76) {
      hgtValid = true;
    }
  }
